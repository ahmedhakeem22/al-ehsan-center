...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateShiftDate(event) {
      var id = event.id;
      var newDate = event.startStr; // YYYY-MM-DD
      var employeeId = event.extendedProps.employee_id;
      var shiftDefinitionId = event.extendedProps.shift_definition_id;
      ...
      fetch('/hr/employee-shifts/' + id, {
        ...
        .then(data => {
          if (!data.success) {
            alert('فشل تحديث المناوبة: ' + (data.message || 'خطأ غير معروف'));
