...
@section('title', 'إنشاء طلب فحص مخبري')

@push('styles')
    {{-- لتحسين شكل حقل الاختيار المتعدد، يفضل استخدام مكتبة مثل Choices.js أو Select2 --}}
...
<div class="page-wrapper">
    <div class="content container-fluid">
        ...
        @component('components.page-header')
            @slot('title') طلبات الفحص المخبري @endslot
            @slot('li_1') <a href="{{ route('clinical.lab_requests.index', $patient->id) }}">قائمة الطلبات</a> @endslot
            @slot('li_2') إنشاء طلب جديد لـ: {{ $patient->full_name }} @endslot
        @endcomponent
            ...
            <div class="col-md-10 offset-md-1">
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title">تفاصيل طلب الفحص الجديد</h5>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('clinical.lab_requests.store', $patient->id) }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="datetime-local" id="request_date" name="request_date" class="form-control @error('request_date') is-invalid @enderror"
                                           value="{{ old('request_date', now()->format('Y-m-d\TH:i')) }}" required>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <select name="requested_tests[]" id="requested_tests" class="form-select @error('requested_tests') is-invalid @enderror" multiple required>
                                        @foreach($availableTests as $test)
                                            <option value="{{ $test->id }}" @selected(in_array($test->id, old('requested_tests', [])))>
                                                {{ $test->name }} ({{ $test->code }})
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-12 mb-3">
                                    ...
                                    <textarea name="notes_from_doctor" id="notes_from_doctor" class="form-control @error('notes_from_doctor') is-invalid @enderror" rows="4">{{ old('notes_from_doctor') }}</textarea>
                                    @error('notes_from_doctor') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>
                            </div>
                            <div class="text-end mt-4">
                                <a href="{{ route('clinical.lab_requests.index', $patient->id) }}" class="btn btn-secondary">إلغاء</a>
